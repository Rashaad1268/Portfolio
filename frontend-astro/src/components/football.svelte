<script lang="ts">
	...
	let ballOffset = 0;
	const offset = -600; // A random value

	function getBallOffset() {
		if (!ball) return 0;

		let _offset = Math.min(
			window.scrollY - ball.getBoundingClientRect().top + offset,
			window.innerWidth - 80 - 90
		);
		return _offset;
	}
...
<div class="hidden lg:flex">
	<svg
		...
		viewBox="0 0 512 512"
		id="football"
		class="z-50 fill-white"
		height="90px"
		width="90px"
		bind:this={ball}
		style="transform: translate({ballOffset > 0
			? ballOffset
			: 0}px, 23rem) rotate({ballOffset}deg);"
	>
