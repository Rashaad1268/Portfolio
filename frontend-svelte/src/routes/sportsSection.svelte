<script lang="ts">
	...
	onMount(async () => {
		...
		ballOffset = !!ball ? window.scrollY - ball.getBoundingClientRect().top + offset : 0;
	});

	function handleScroll() {
		if (!ball) return; // ball can be null on re-renders
		ballOffset = window.scrollY - ball.getBoundingClientRect().top + offset;
	}
</script>
...
<div class="hidden lg:flex">
	...
	<svg
		...
		id="football"
		bind:this={ball}
		style="transform: translate(calc(17rem + {ballOffset > 0
			? ballOffset
			: 0}px), 11rem) rotate({ballOffset}deg);"
	>
...
<style lang="postcss">
	...
	#football {
		@apply absolute z-50 h-[60px] fill-white;
	}
</style>
